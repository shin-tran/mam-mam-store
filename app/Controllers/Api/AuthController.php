...
class AuthController {
  ...
  public function handleRegister() {
    ...
    if (!$userId && !$isSetRole) {
      Helpers::sendJsonResponse(
        false,
        'Đăng ký thất bại do lỗi hệ thống. Vui lòng thử lại!',
        null,
        500
      );
    }
  ...
  public function handleLogin() {
    ...
    $isSaved = $tokenModel->saveToken(
      $user['id'],
      $refreshTokenHash,
      $refreshTokenExpiresAt->format('Y-m-d H:i:s'),
      $userAgent,
      $ipAddress
    );
    ...
    if (!$isSaved) {
      Helpers::sendJsonResponse(
        false,
        'Lỗi hệ thống, không thể tạo phiên đăng nhập.',
        null,
        500
      );
    }
  ...
  public function activateAccount() {
    ...
    if (!$isActivated)
      Helpers::sendJsonResponse(
        false,
        'Có lỗi xảy ra trong quá trình kích hoạt.',
        null,
  ...
  public function handleForgotPassword() {
    ...
    if (!$user)
      Helpers::sendJsonResponse(
        false,
        'Email chưa được đăng ký hoặc không chính xác.',
        null,
    ...
    if (!$isTokenSet)
      Helpers::sendJsonResponse(
        false,
        'Có lỗi hệ thống xảy ra, xin vui lòng thử lại sau!',
        null,
        500
      );
