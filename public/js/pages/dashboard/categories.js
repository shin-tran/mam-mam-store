...
const categoryNameToDeleteSpan = document.getElementById("category-name-to-delete");
...
addCategoryBtn?.addEventListener("click", () => {
    if (!categoryModal || !categoryModalTitle || !categoryForm)
        return;
...
categoriesTable?.addEventListener("click", (event) => {
    ...
    if (deleteButton) {
        ...
        if (!deleteCategoryModal || !categoryNameToDeleteSpan)
            return;
...
confirmDeleteCategoryBtn?.addEventListener("click", async (event) => {
    ...
    try {
        const response = await authService.fetchWithAuth(`/api/categories/delete/${categoryIdToDelete}`, {
            method: "POST",
        });
        const result = await response.json();
        if (result.success) {
            toastManager.createToast({ message: result.message, type: "success" });
            const rowToDelete = categoriesTable?.querySelector(`tr[data-category-id="${categoryIdToDelete}"]`);
            rowToDelete?.remove();
        }
        else {
            toastManager.createToast({ message: result.message, type: "error" });
