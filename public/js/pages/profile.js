...
async function loadProvinces() {
    if (!provinceSelect)
        return;
    try {
        const response = await fetch("https://tinhthanhpho.com/api/v1/new-provinces?limit=36");
        const result = await response.json();
        if (result.success && result.data) {
...
async function handleProvinceChange() {
    if (!provinceSelect || !wardSelect || !provinceCodeInput)
        return;
    ...
    try {
        const response = await fetch(`https://tinhthanhpho.com/api/v1/new-provinces/${provinceCode}/wards?limit=170`);
        const result = await response.json();
        if (result.success && result.data) {
            wards = result.data;
            wardSelect.innerHTML = '<option value="">-- Chọn Phường/Xã --</option>';
...
function renderAddresses(addresses) {
    ...
    addressesList.innerHTML = addresses
        ...
        return `
        <div class="card bg-base-200 ${isDefault ? "border-2 border-primary" : ""}">
            ...
            : ""}
              ...
              <div class="dropdown dropdown-end">
                ...
                <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-52">
                  <li>
                    <button class="edit-address-btn" data-address-id="${address.id}">
...
async function openEditAddressModal(addressId) {
    if (!addressModal || !addressForm)
        return;
    try {
        const response = await authService.fetchWithAuth(`/api/users/addresses/${addressId}`);
        const result = await response.json();
        if (result.success && result.data) {
            ...
            await loadProvinces();
            addressForm.elements.namedItem("recipient_name").value = address.recipient_name;
            addressForm.elements.namedItem("phone_number").value = address.phone_number;
            addressForm.elements.namedItem("street_address").value = address.street_address;
            // Set city/province
            if (provinceSelect) {
                // Find and select the matching province
                const options = Array.from(provinceSelect.options);
                const matchingOption = options.find((opt) => opt.value === address.city);
                if (matchingOption) {
            ...
            addressForm.elements.namedItem("is_default").checked = address.is_default === 1;
...
async function handleDeleteAddress(addressId) {
    if (!confirm("Bạn có chắc chắn muốn xóa địa chỉ này?"))
        return;
    try {
        const response = await authService.fetchWithAuth(`/api/users/addresses/delete/${addressId}`, { method: "POST" });
        const result = await response.json();
        if (result.success) {
            toastManager.createToast({
...
async function handleSetDefaultAddress(addressId) {
    try {
        const response = await authService.fetchWithAuth(`/api/users/addresses/set-default/${addressId}`, { method: "POST" });
        const result = await response.json();
        if (result.success) {
            toastManager.createToast({
